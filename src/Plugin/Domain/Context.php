...
class Context
{
    ...
    private function validate(string $jsonFileContent): object
    {
        ...
        if (
            $json->type == 'aikeedo-plugin'
            && (!isset($json->extra->{'entry-class'}) || !$json->extra->{'entry-class'})
        ) {
            throw new InvalidPluginComposerJsonFileException(
                "Invalid composer.json file: <extra.entry-class> is not set."
            );
        }
    ...
    private function populate(object $json): void
    {
        ...
        // @phpstan-ignore-next-line
        $this->releasedAt = $json->time ? new DateTime($json->time) : null;
        ...
        // @phpstan-ignore-next-line
        $this->entryClass = new EntryClass($json->extra->{'entry-class'} ?? null);
